...
export default class App extends Component {
  ...
  changeData() {
    //    const airportInfo = [...this.state.airportInfo];
    //    airportInfo[0].temperature += 1;
    //    this.setState({ airportInfo });
    
    //    const airportInfo = this.state.airportInfo.map(airport =>
    //      new Airport(
    //        airport.code, airport.name, airport.temperature + 1, airport.delay));
    //    //Vioilates both the DRY principle and the Open Closed Principle
    //    this.setState({ airportInfo });

    const airportInfo = this.state.airportInfo.map(airport =>
      Object.assign(new Airport(), airport, { temperature: 
          airport.temperature + this.toggle }));
  ...
  render() {
    return <div>
      ...
      <DisplayAirportStatus data={ this.state.airportInfo } />
    </div>;
  }
}