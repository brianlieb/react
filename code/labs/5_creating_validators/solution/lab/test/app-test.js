...
describe('app test', function() {
  ...
  it('App passes airportInfo to DisplayAirportStatus', () => {
    ...
    expect(displayAirportStatus1.props().data).to.be.eql(wrapper.state().airportInfo);
    expect(displayAirportStatus1.props().delayed).to.be.eql(true);

    const displayAirportStatus2 = wrapper.find('DisplayAirportStatus').at(1);
    expect(displayAirportStatus2.props().data).to.be.eql(wrapper.state().airportInfo);
  ...
  it('App passes title to DisplayAirportStatus', () => {
    ...
    const displayAirportStatus1 = wrapper.find('DisplayAirportStatus').at(0);
    expect(displayAirportStatus1.props().title).to.be.eql('Airports With Delays');

    const displayAirportStatus2 = wrapper.find('DisplayAirportStatus').at(1);
    expect(displayAirportStatus2.props().title).to.be.eql('Airports With No Delays');
  });
});